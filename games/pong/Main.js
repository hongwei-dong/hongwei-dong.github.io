...
function update()
{
	...
	if(ball.x + 30 > cpu.x && ball.x + 30 < cpu.x + paddleWidth && ball.y >= cpu.y && ball.y < cpu.y + 75)
	{
		xSpeed *= -1;
		SoundJS.play('hit');
	}
	...
	if(ball.x <= player.x + paddleWidth && ball.x > player.x && ball.y >= player.y && ball.y < player.y + 75)
	{
		xSpeed *= -1;
		SoundJS.play('hit');
	}
